...
有了这个设计之后，共享Volume就简单多了：Kubernetes 项目只要++把所有Volume 的定义都设计在 Pod 层级即可++。这样，一个 Volume对应的宿主机目录对于Pod来说就只有一个，Pod 里的容器只要声明挂载这个 Volume，就一定可以共享这个Volume对应的宿主机目录。比如下面这个例子：
