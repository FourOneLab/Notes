...
```
> 需要主要的是，Pod仅设置了limits没有设置requests的时候，kubernetes会自动为它设置与limits相通的requests值，所有也属于Guaranteed类别。
...
**QoS的划分主要的应用场景是，当宿主机资源紧张的时候，kubelet对Pod进行Eviction（即资**源回收**）时候需要用到。**
...
- 其次是Burstable类别，并且发生“饥饿”的资源使用量已经查出requests的Pod
- 最后是Guaranteed类别：
...
# 总结
DaemonSet的Pod都设置为Guaranteed的QoS类型，否则一旦DaemonSet的Pod被回收，它优惠立即在原宿主机上被重建出来，这就使得前面资源回收的动作，完全没有意义了。
